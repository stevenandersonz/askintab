...
(async () => {
  ...
  class ChatUI {
    ...
    getStyles() {
      ...
      return `
        ...
        #${SIDE_CHAT_CONTAINER_ID}.chat-hidden {
    ...
    initializeElements() {
      this.elements = {
        ...
        badgeMenuItems: $$('.badge-menu-item', this.root) // Get menu items here
      };
    }
    ...
    bindEvents() {
      this.elements.closeBtn?.addEventListener('click', () => this.toggleChatVisibility(false));
      this.elements.sendBtn?.addEventListener('click', () => this.handleSendMessage());
      this.elements.input?.addEventListener('keypress', this.handleInputKeyPress.bind(this));
      this.elements.input?.addEventListener('input', this.updateSendButtonState.bind(this)); // Update on input
    ...
    toggleChatVisibility(show) {
        ...
        const bodyPadding = show ? SIDE_CHAT_WIDTH : '0';

        if (!container) return;

        // Use transitions for smooth slide-in/out
  ...
  class SideChat {
    ...
    async initialize() {
      ...
      this.ui.render(); // Render UI elements

      this.setupMessageHandling();
      this.ui.toggleChatVisibility(true); // Show chat by default
    ...
    setupMessageHandling() {
      chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
        // Ensure messages are from the extension background script (optional but good practice)
        // if (sender.id !== chrome.runtime.id) return false;

        if (msg.action === "RESPONSE") {
          this.handleResponse(msg.payload);
          return true; // Indicate async response potentially handled later
        }
  ...
  // --- Initialization ---
  // Use a flag to prevent multiple initializations from rapid script injections
  if (!window.hasInitializedAskInTabSideChat) {
      ...
      if (typeof mermaid === 'undefined') {
          ...
          // Potentially inject mermaid library here if needed and allowed
      }
      if (typeof marked === 'undefined') {
          console.warn(`${EXT_NAME}: Marked library not found. Markdown may not render correctly.`);
          // Potentially inject marked library here if needed and allowed
      }
